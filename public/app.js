// Mermaid — render diagrams inline; securityLevel strict prevents script injection
...
async function renderMermaidDiagram(source) {
  const container = el('r-diagram-rendered');
  if (!source) { container.innerHTML = ''; return; }
  try {
    // mermaid.render requires a unique id each call to avoid caching issues
    const id = 'mermaid-svg-' + Date.now();
    const { svg } = await mermaid.render(id, source);
    // svg is generated by mermaid from our hard-coded DSL — not user input
...
document.getElementById('toggle-source-btn').addEventListener('click', () => {
  const pre = el('r-diagram');
  const btn = document.getElementById('toggle-source-btn');
  const hidden = pre.classList.toggle('hidden');
  btn.textContent = hidden ? 'Show source' : 'Hide source';
});
